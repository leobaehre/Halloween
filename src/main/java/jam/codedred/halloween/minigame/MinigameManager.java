...
public class MinigameManager {
    ...
    public static void switchToMiniGame(Minigame minigame) {
        if (currentMinigame != null) currentMinigame.endGame();
        minigame.updateScoreboard();
        minigame.startGame();
        currentMinigame = minigame;
        roundMinigameList.remove(minigame);
        //ScoreboardManager.sortScoreboard();
    }

    public static Minigame getNextMinigame() {
        if (roundMinigameList.isEmpty()) return null;
        return roundMinigameList.get(0);
    }

    public static void selectGamesForRound(int amount) {
    ...
    public static void startRound(Collection<? extends Player> players) {

        selectGamesForRound(3);

        Minigame nextGame = getNextMinigame();

        for (Player player : players) {
            setPlayerCandies(player, 0);
            ScoreboardManager.setScoreboard(player);
        }

        ScoreboardManager.sortScoreboard();

        switchToMiniGame(nextGame);

        System.out.println("Started Round with minigame " + nextGame.name);
    }
